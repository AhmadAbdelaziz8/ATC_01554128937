        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4 md:gap-6">
          {categories.map((category) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mb-3 transition-all duration-200 group-hover:shadow-lg ${category.bgColor}`}
                >
                  <IconComponent
                ...
                <span className="text-sm sm:text-base font-medium text-slate-700 group-hover:text-orange-500">
                  {category.name}
                </span>
              </Link>
            );
